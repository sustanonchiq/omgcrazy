var Main = {
	...
	fitUsername: function ($el) {
		$el.each(function () {
			...
			while (this.scrollWidth > this.offsetWidth && prefix_len > 3) {
				prefix_len--
	...
	initViewport: function () {
		if (!window.$viewportHelper) {
			window.$viewportHelper = $('<div>')
				.css({ position: 'absolute', left: '-100px', top: '0', height: '100vh' })
				.appendTo('body')
		}
		Main.onResize()
	},
	...
	amountFieldValue: function ($form, field) {
		...
		if ((minValue !== null && float_value < minValue) || (maxValue !== null && float_value > maxValue)) {
			return false
		} else {
			return float_value
		}
	...
	eUpdateAmountField: function (e) {
		...
		for (var i = 0; i < value.length; i++) {
			...
			if ((char == '.' || char == ',') && !has_decimal && decimals > 0) {
				...
				char <= '9' &&
				chars_len < 12 &&
				(!has_decimal || decimal_len < decimals)
			) {
				new_value += char
		...
		if (
			(minValue !== null && float_value < minValue) ||
...
var Assets = {
	...
	eTableRowSelHovered: function (e) {
		$(this)
			.closest('.tm-row-selectable')
			.toggleClass('noselect', e.type == 'mouseover')
	},
	eGetCode: function (e) {
		e.stopImmediatePropagation()
		e.preventDefault()
	...
	eAssignToTelegram: function (e) {
		...
		openPopup(Aj.state.$assignPopup, {
			onOpen: function () {
				...
				if (assigned_to) {
					$(
						'.js-assign-account-item:has(input[name="assign_to"][value="' + assigned_to + '"])',
						Aj.state.$assignPopup,
					).each(function () {
	...
	eBotUsernameSubmit: function (e) {
		...
		Wallet.sendTransaction({
			...
			description: l('WEB_POPUP_QR_BOT_USERNAME_TEXT', {
				amount:
					'<span class="icon-before icon-ton-text js-amount_fee">' + Aj.state.botUsernameFee + '</span>',
			}),
...
var Account = {
	...
	convertStateCheck: function (req_id, force) {
		...
		clearTimeout(Aj.state.convertTimeout)
		Aj.state.convertTimeout = setTimeout(
			function () {
				Aj.apiRequest(
					'checkConverting',
					{
...
var PremiumGiveaway = {
	init: function () {
		Aj.onLoad(function (state) {
			...
			state.$premiumSearchForm.on(
				'change',
				'.js-premium-options input.radio',
				PremiumGiveaway.eRadioChanged,
			)
	...
	eGiveawayPremiumSubmit: function (e) {
		...
		Wallet.sendTransaction({
			...
			description: l('WEB_POPUP_QR_GIVEAWAY_TEXT', {
				amount:
					'<span class="icon-before icon-ton-text js-amount_fee">' + Aj.state.giveawayPrice + '</span>',
			}),
			qr_label: item_title,
...
var Ads = {
	...
	updateWithdrawState: function (force) {
		...
		if (
			...
			Aj.state.updLastReq = now
			Aj.apiRequest(
				'updateAdsRevenueWithdrawalState',
				{
			...
			function (result) {
				...
				Wallet.sendTransaction({
					...
					description: l('WEB_POPUP_QR_ADS_RECHARGE_TEXT', {
						amount:
							'<span class="icon-before icon-ton-text js-amount_fee">' + result.amount + '</span>',
					}),
...
var Stars = {
	...
	toggleOptions: function (expand) {
		Aj.state.$starsSearchForm
			.toggleClass('options-expanded', expand)
			.toggleClass('options-collapsed', !expand)
	},
	eRadioChanged: function () {
		Stars.clearQuantity()
	...
	eBuyStarsSubmit: function (e) {
		...
		Wallet.sendTransaction({
			...
			description: l('WEB_POPUP_QR_STARS_TEXT', {
				amount:
					'<span class="icon-before icon-ton-text js-amount_fee">' + Aj.state.starsPrice + '</span>',
			}),
			qr_label: item_title,
	...
	updateWithdrawState: function (force) {
		...
		if (
			...
			Aj.state.updLastReq = now
			Aj.apiRequest(
				'updateStarsRevenueWithdrawalState',
				{
					transaction: Aj.state.transaction,
...
var QR = {
	showPopup: function (options) {
		...
		var ua = (navigator.userAgent || '').toLowerCase(),
			is_ios = ua.indexOf('iphone') >= 0 || ua.indexOf('ipad') >= 0,
			is_android = ua.indexOf('android') >= 0
		var qr_label = options.qr_label
			? '<div class="tm-qr-code-label fit-text">' + options.qr_label + '</div>'
			: ''
		var hint = options.hint ? '<p class="popup-text popup-hint-text">' + options.hint + '</p>' : ''
		var tk_button =
			options.tk_label && (is_android || is_ios)
				? '<p class="tm-qr-code-or">' +
				  l('WEB_POPUP_QR_OR_BUTTON') +
				  '</p><button class="btn btn-primary btn-block btn-tonkeeper js-tonkeeper-btn" data-inactive-label="' +
				  l('WEB_WAITING', 'Waiting...') +
				  '"><span class="tm-button-label">' +
				  options.tk_label +
				  '</span></button>'
				: ''
		var terms_text = options.terms_label
			? '<p class="popup-footer-text">' + options.terms_label + '</p>'
			: ''
		var $popup = $(
			'<div class="popup-container hide qr-code-popup-container' +
				(options.popup_class ? ' ' + options.popup_class : '') +
				' qr-inactive" data-close-outside="popup-body"><div class="popup"><div class="popup-body"><section><h2>' +
				options.title +
				'</h2><p class="popup-text">' +
				options.description +
				'</p><div class="tm-qr-code"><div class="tm-qr-code-image"></div>' +
				qr_label +
				'</div>' +
				tk_button +
				hint +
